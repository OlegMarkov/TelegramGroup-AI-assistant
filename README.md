...
- `/summary [hours]` — AI-generated summary of a group's recent activity. With no number it covers **everything since your last summary of that chat**, falling back to 24h the first time. Run inside a group to summarize it directly, or in DM to pick from your linked groups.
...
## "Since I last checked" as the default

`/summary` with no argument used to assume 24h, which made the user guess a number the bot already knew the answer to. It now looks back to the last summary it actually **delivered** to that user for that chat, recorded per (user, chat) in `summary_reads`. A first request for a chat still falls back to 24h. The header says which of the two happened, so `last 7h` is never an unexplained number, and an explicit `/summary 6` is unchanged.

**The computed window is rounded up, to whole hours.** Both halves matter. *Up*, because "since you last checked" must never silently drop the most recent minutes of conversation — over-covering by up to an hour is the safe direction. *Whole*, because the digest cache is keyed by `(chat_id, hours, language)`: a per-user window carrying fractions would give every requester their own cache row and their own DeepSeek call. Two people whose last reads are half an hour apart round to the same window and share one cached summary, which is what keeps a per-user default from multiplying the bill.

**It cannot become an uncapped lookback.** The computed window goes through the same `Math.min(rawHours, limits.maxLookbackHours)` an explicit argument does, so somebody who has been away a fortnight is still clamped to 24h or 72h by plan and told so by the existing cap note. Where the 200-message cap then cuts the window, the `truncated` reporting below says so, however the hours were computed.

**The read is recorded only after delivery succeeds**, immediately before the handler returns — deliberately not alongside `incrementSummaryUsage`, which runs earlier. A daily quota counted too early self-corrects within 24h; a read recorded for a summary that never arrived would skip that content permanently, because every later "since you last checked" would start from a timestamp covering messages the user never saw.

`summary_reads` is a usage counter like `daily_usage`, and `deleteUserData` drops it with the rest on `/forgetme`.

## Weekly digests, and the message cap
